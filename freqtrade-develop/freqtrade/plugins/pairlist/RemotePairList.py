...
import rapidjson
import requests
from freqtrade.util.http_retry import http_get_json
    ...
    def fetch_pairlist(self) -> tuple[list[str], float]:
        ...
        try:
            ...
            response_json = http_get_json(
                self._pairlist_url, headers=headers, timeout=self._read_timeout, retries=3
            )
            time_elapsed = time.perf_counter() - start
            try:
                pairlist = self.process_json(response_json)
