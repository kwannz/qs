...
AVAILABLE_CLI_OPTIONS = {
    ...
    "fill_gaps": Arg(
        "--fill-gaps",
        help="Attempt to fill OHLCV gaps (requires --exchange).",
        action="store_true",
    ),
    "jobs": Arg(
        "-j",
        "--jobs",
        help="Number of parallel jobs to use (data-quality).",
        type=check_int_positive,
        metavar="INT",
    ),
    "fix": Arg(
        "--fix",
        help="Attempt to fix detected issues (currently: deduplicate only).",
        action="store_true",
    ),
    "logfile": Arg(
        "--logfile",
