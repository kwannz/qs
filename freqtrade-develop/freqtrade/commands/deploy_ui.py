...
import requests
from freqtrade.util.http_retry import http_get_bytes, http_get_json
...
def download_and_install_ui(dest_folder: Path, dl_url: str, version: str):
    ...
    resp = http_get_bytes(dl_url, timeout=req_timeout, retries=3)
...
def get_ui_download_url(version: str | None, prerelease: bool) -> tuple[str, str]:
    ...
    r = http_get_json(f"{base_url}releases", timeout=req_timeout, retries=3)
    ...
    if not dl_url:
        ...
        r = http_get_json(assets, timeout=req_timeout, retries=3)
