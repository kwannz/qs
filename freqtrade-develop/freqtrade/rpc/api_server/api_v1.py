...
def trades(
    ...
    fields: list[str] | None = Query(
        None,
        description="Optional list of fields to include per trade (payload slimming)",
    ),
    rpc: RPC = Depends(get_rpc),
):
    data = rpc._rpc_trade_history(limit, offset=offset, order_by_id=order_by_id)
    if fields:
        fld = set(fields)
        try:
            return [{k: v for k, v in d.items() if k in fld} for d in data]
        except Exception:
            return data
    return data
...
def list_open_trades_custom_data(
    ...
    """
    try:
        data = rpc._rpc_list_custom_data(key=key, limit=limit, offset=offset)
        if fields:
            fset = set(fields)
            # data is a list of dicts
            data = [{k: v for k, v in d.items() if k in fset} for d in data]
        return data
...
def list_custom_data(
    ...
    """
    try:
        data = rpc._rpc_list_custom_data(trade_id, key=key)
        if fields:
            fset = set(fields)
            data = [{k: v for k, v in d.items() if k in fset} for d in data]
        return data
