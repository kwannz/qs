...
class ApiServer(RPCHandler):
    ...
    def __init__(self, config: Config, standalone: bool = False) -> None:
        ...
        # Warn if JWT secret is default - login endpoint will still reject default secrets.
        jwt_secret = api_config.get("jwt_secret_key", "super-secret")
        if jwt_secret in ("super-secret", "super-secret, somethingrandom"):
            logger.warning(
                "SECURITY WARNING - 'jwt_secret_key' appears to use a default value."
                " Token-based login will be rejected until this is changed."
            )
    ...
    def configure_app(self, app: FastAPI, config):
        ...
        from freqtrade.rpc.api_server.api_auth import require_admin_for_write
        ...
        # Public API with light rate-limit
        rl_backend = config["api_server"].get("rate_limit_backend", "memory")
        rl_redis = config["api_server"].get("rate_limit_redis_url")
        app.include_router(api_v1_public, prefix="/api/v1")

        app.include_router(router_login, prefix="/api/v1", tags=["auth"])
        # Private API: auth + rate-limit + RBAC (admin required for writes)
        if config["api_server"].get("auth_disabled", False):
            private_deps = []
        else:
            private_deps = [Depends(http_basic_or_jwt_token), Depends(require_admin_for_write)]
        if private_deps and config["api_server"].get("enable_rate_limit", False):
            private_deps.insert(
                1,
                Depends(
                    token_or_ip_rate_limiter(
                        limit=config["api_server"].get("rate_limit_per_minute", 120),
                        window_sec=60,
                        backend=rl_backend,
                        redis_url=rl_redis,
                    )
        ...
        app.include_router(api_v1, prefix="/api/v1", dependencies=private_deps)
        app.include_router(
            api_backtest,
            prefix="/api/v1",
        ...
        # Harden CORS: disallow wildcards, only allow explicit origins
        cors_origins = config["api_server"].get("CORS_origins", []) or []
        if any(origin in ("*", "*") for origin in cors_origins):
            logger.warning("SECURITY WARNING - Wildcard CORS origin removed. Configure explicit origins.")
            cors_origins = [o for o in cors_origins if o not in ("*", "*")]
