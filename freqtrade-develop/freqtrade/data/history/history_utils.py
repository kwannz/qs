...
def _normalize_ohlcv_timezone_and_boundary(
    df: DataFrame, timeframe: str, *, fix_tz: bool = True
) -> tuple[DataFrame, dict[str, int]]:
    """
    Ensure OHLCV 'date' column is tz-aware UTC and warn for misaligned candle boundaries.
    Returns (possibly adjusted df, stats dict)
    """
    stats = {"tz_naive": 0, "misaligned": 0}
    ...
    # tz-aware check
    try:
        tzinfo = getattr(dts.dt.tz, "zone", None)
    except Exception:
        tzinfo = None
    if tzinfo is None and dts.dt.tz is None:
        stats["tz_naive"] = len(df)
        if fix_tz:
            df = df.copy()
            df["date"] = dts.dt.tz_localize("UTC")
            dts = df["date"]
            logger.debug("Localized naive OHLCV timestamps to UTC")

    # Boundary alignment check
    try:
        step = timeframe_to_seconds(timeframe)
        ts = (dts.astype("int64") // 10**9).astype("int64")
        misaligned_mask = (ts % step) != 0
        stats["misaligned"] = int(misaligned_mask.sum())
        if stats["misaligned"] > 0:
            logger.warning(
                "Detected %s misaligned candles for timeframe %s (off-boundary data).",
                stats["misaligned"],
                timeframe,
            )
    except Exception:
...
def load_pair_history(
    ...
    *,
    ...
    # Normalize timezone and check boundaries
    df, _ = _normalize_ohlcv_timezone_and_boundary(df, timeframe)
    return df
