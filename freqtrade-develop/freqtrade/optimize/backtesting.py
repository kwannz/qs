...
class Backtesting:
    ...
    def backtest_one_strategy(
        ...
        # Optional Rust-FFI engine (experimental): toggled via env FT_USE_RUST_FFI_BACKTEST
        results: BacktestContentTypeIcomplete
        use_python = True
        try:
            from freqtrade.optimize import rustffi
            if rustffi.enabled():
                logger.info("Using Rust FFI backtest engine (experimental)")
                from typing import cast
                results = cast(
                    BacktestContentTypeIcomplete,
                    rustffi.run(self, preprocessed, min_date, max_date),
                )
                use_python = False
        except Exception as e:
            logger.warning(
                f"Rust FFI backtest unavailable or failed, falling back. Reason: {e}"
            )
