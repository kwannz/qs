...
> If you use Freqtrade strictly on a personal machine, keep all services bound to `127.0.0.1` and avoid exposing ports publicly. For this use-case, you may disable API auth locally. When moving to remote/multi-user environments, re-enable authentication (JWT/Basic), configure CORS and consider Redis-backed rate limits.
