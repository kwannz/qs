...
def get_user_factor_list(
    ...
    try:
        # 验证排序参数
        valid_sort_fields = [
            "updated_at", "created_at", "return_ratio", "annualized_ratio",
            "sharpe_ratio", "maximum_drawdown", "IC", "IR"
        ]
        ...
        # 按排序字段获取数据
        base_sort_fields = {"created_at", "updated_at"}
        sort_dir = -1 if sort_order == "desc" else 1
        if sort_field in base_sort_fields:
            # 数据库端排序与分页（仅限基础字段）
            cursor = (
                _db_handler.mongo_client["panda"]["user_factors"]
                .find(query)
        ...
        if sort_field in base_sort_fields:
            ...
            # 分析指标排序：兼容字符串百分比与数值
            def sort_key(it: UserFactorListItem):
                val = getattr(it, sort_field)
                # 百分比字符串转数值
...
def run_factor(message_id: str,is_thread: bool):
    try:
        logger.debug(f"run_factor called with message_id={message_id}")
