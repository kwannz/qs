...
def factor_analysis(df_factor: pd.DataFrame, params: Params, factor_id: str = "", task_id: str = "",
    ...
    factor_name = None
    if factor_id:
        # 兼容字符串/ObjectId 两种 _id 形式
        _query_id = None
        try:
            _query_id = ObjectId(factor_id)
        except Exception:
            _query_id = factor_id
        factors = _db_handler.mongo_find("panda", "user_factors", {"_id": _query_id})
    ...
    try:
        ...
        try:
            ...
            logger.debug(msg=f"K-line data tail:\n{df_k_data.tail(5) if df_k_data is not None else 'K-line data is None'}")
        ...
        try:
            ...
            logger.debug(msg=f"Merged dataframe length: {len(df)}")
            ...
            logger.error(msg=error_msg)
            raise
        logger.debug(msg=f"Merged dataframe tail:\n{df.tail(5)}")
