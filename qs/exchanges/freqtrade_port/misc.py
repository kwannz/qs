...
import pandas as pd
try:
    import rapidjson as jsonlib
except ImportError:  # Fallback to stdlib json if rapidjson not available
    import json as jsonlib

from qs.exchanges.freqtrade_port.enums import SignalTagType, SignalType
...
def dump_json_to_file(file_obj: TextIO, data: Any) -> None:
    ...
    """
    # Use rapidjson if available for speed, otherwise stdlib json
    try:
        jsonlib.dump(data, file_obj, default=str, number_mode=jsonlib.NM_NATIVE)  # type: ignore
    except Exception:
        jsonlib.dump(data, file_obj, default=str)
...
def json_load(datafile: TextIO) -> Any:
    ...
    """
    try:
        return jsonlib.load(datafile, number_mode=jsonlib.NM_NATIVE)  # type: ignore
    except Exception:
        return jsonlib.load(datafile)
