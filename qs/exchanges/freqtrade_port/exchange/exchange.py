...
from qs.exchanges.freqtrade_port.configuration import remove_exchange_credentials
from qs.exchanges.freqtrade_port.constants import (
...
from qs.exchanges.freqtrade_port.data.converter import (
...
from qs.exchanges.freqtrade_port.enums import (
    OPTIMIZE_MODES,
    TRADE_MODES,
    CandleType,
    MarginMode,
...
from qs.exchanges.freqtrade_port.exceptions import (
    ConfigurationError,
    DDosProtection,
    ExchangeError,
...
from qs.exchanges.freqtrade_port.exchange.common import (
    API_FETCH_ORDER_RETRY_COUNT,
    retrier,
    retrier_async,
)
from qs.exchanges.freqtrade_port.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from qs.exchanges.freqtrade_port.exchange.exchange_ws import ExchangeWS
from qs.exchanges.freqtrade_port.misc import (
    chunks,
    deep_merge_dicts,
    file_dump_json,
    file_load_json,
...
from qs.exchanges.freqtrade_port.util.datetime_helpers import dt_from_ts, dt_now
from qs.exchanges.freqtrade_port.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
from qs.exchanges.freqtrade_port.util.periodic_cache import PeriodicCache
...
class Exchange:
    ...
    def __init__(
        ...
        *,
        ...
        # Recent market metadata change events (bounded)
        self._market_change_events: list[dict[str, Any]] = []
    ...
    def reload_markets(self, force: bool = False, *, load_leverage_tiers: bool = True) -> None:
        ...
        try:
            ...
            old_markets = self._markets or {}
            new_markets = self._api_async.markets
            # Detect and log significant changes
            if old_markets:
                changes = self._detect_market_changes(old_markets, new_markets)
                if changes:
                    logger.warning(
                        "Market metadata changed for %s pairs (precision/limits).",
                        len(changes),
                    )
                    # Store last few change summaries
                    ts = dt_now()
                    for ch in changes[:50]:
                        self._market_change_events.append({"ts": ts, **ch})
                    # Bound list size
                    self._market_change_events = self._market_change_events[-200:]
    ...
    def _detect_market_changes(
        ...
        """
        Compare important market metadata between 2 snapshots and return list of changes.
        Relevant fields: precision (amount, price, cost), limits.amount.min/max, limits.cost.min/max,
        lot, contractSize, active.
        """
        fields = [
            ("precision", "amount"),
            ("precision", "price"),
        ...
        def _get(d: dict, path: tuple | None, leaf: str | None = None):
            if path is None:
                return d.get(leaf) if leaf else None
            cur = d
            for p in path:
                if isinstance(p, str):
        ...
        # Only consider pairs present in both snapshots
        ...
        for sym in common:
            ...
            for entry in fields:
                if entry[0] is None:
                    ...
                    try:
                        for p in path:
                            ov = ov.get(p, {}) if isinstance(ov, dict) else {}
                            nv = nv.get(p, {}) if isinstance(nv, dict) else {}
                        ovv = ov.get(leaf)
                        nvv = nv.get(leaf)
                        if ovv != nvv:
                            key = ".".join([*path, leaf])  # type: ignore[arg-type]
                            diff[key] = {"old": ovv, "new": nvv}
                    except Exception:
                        # Ignore path issues gracefully
                        continue
            if diff:
                changes.append({"symbol": sym, "changes": diff})
        return changes
    ...
    def fetch_dry_run_order(self, order_id) -> CcxtOrder:
        ...
        try:
            ...
            from qs.exchanges.freqtrade_port.persistence import Order
    ...
    def create_order(
        ...
        *,
        ...
        # Dry-run should not enforce exchange-level min limits to keep tests/backtests stable.
        ...
        try:
            ...
            # Validate after precision rounding (non-dry run)
            base_amount = self._contracts_to_amount(pair, amount)
            self._validate_min_limits_base(pair, base_amount, rate_for_order)
    ...
    def _validate_min_limits_base(self, pair: str, base_amount: float, rate: float | None) -> None:
        """
        Centralized validation for min amount and min cost requirements in base/quote units.
        Raises InvalidOrderException with a unified, actionable message.
        """
        try:
            market = self.markets[pair]
        except KeyError:
        ...
        # Check minimal base amount
        amt_limits = limits.get("amount") or {}
        min_amt = amt_limits.get("min")
        if min_amt is not None:
            min_base = self._contracts_to_amount(pair, min_amt)
            if base_amount < min_base:
                errors.append(f"amount {base_amount} < min_amount {min_base}")

        # Check minimal notional (spot only; futures handled via tiers)
        cost_limits = limits.get("cost") or {}
        min_cost = cost_limits.get("min")
        if min_cost is not None and rate is not None:
            notional = base_amount * rate
            if notional < min_cost:
                errors.append(f"notional {notional} < min_notional {min_cost}")

        # Futures: leverage tier based minimal notional (best-effort)
        if (
            self.trading_mode == TradingMode.FUTURES
            and rate is not None
            and min_cost is None
            and self._config.get("exchange", {}).get("strict_futures_min_notional", False)
        ):
            try:
                notional = base_amount * rate
                min_notional_tier = self._get_min_notional_from_tiers(pair)
                if min_notional_tier and notional < min_notional_tier:
                    errors.append(
                        f"notional {notional} < min_notional_by_tier {min_notional_tier}"
                    )
            except Exception:
                # be tolerant
                pass
        ...
        if errors:
            raise InvalidOrderException(
                "Order validation failed: " + "; ".join(errors) + ". "
                "Consider increasing stake or adjusting price to meet exchange limits."
            )

    def _get_min_notional_from_tiers(self, pair: str) -> float | None:
        """
        Derive minimal notional from leverage tiers for the given pair.
        Picks the smallest positive 'minNotional' across tiers if available.
        """
        tiers = self._leverage_tiers.get(pair)
        if not tiers:
            return None
        vals = [t.get("minNotional") for t in tiers if t.get("minNotional")]
        if not vals:
            return None
        mn = min([float(v) for v in vals if float(v) > 0.0], default=None)
        return mn
    ...
    def refresh_latest_trades(
        ...
        *,
        ...
        :return: Dict of [{(pair, timeframe): Dataframe}]
        """
        from qs.exchanges.freqtrade_port.data.history import get_datahandler
